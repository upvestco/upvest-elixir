defmodule Upvest.HistoricalDataTest do
  ...
  test "get transactions by address" do
    to_addr = "0x6590896988376a90326cb2f741cb4f8ace1882d5"
    {:ok, tx} = Historical.get_transaction(@client, @protocol, @network, to_addr)
    assert is_list(tx.result)
  end

  test "get asset balance" do
    to_addr = "0x93b3d0b2894e99c2934bed8586ea4e2b94ce6bfd"
    balance = Historical.get_balance(@client, @protocol, @network, to_addr)
  ...
  test "get contract balance" do
    ...
    balance = Historical.get_balance(@client, @protocol, @network, to_addr, contract_addr)
    assert balance.address
    assert balance.contract == nil
    assert balance.address == to_addr
  end
  ...
  test "get status" do
    status = Historical.api_status(@client, @protocol, @network)
