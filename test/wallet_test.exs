defmodule Upvest.WalletTest do
 ...
 test "list all wallets"  do
   ...
   object_keys = Enum.reduce(wallets, [], &(Map.keys(&1) ++ &2)) |> Enum.uniq()
   assert object_keys == ["address", "balances", "id", "index", "protocol", "status"]
 end
 ...
 test "sign wallet", context do
   ...
   {:ok, _resp} = Wallet.sign(@client, context.wallet["id"], to_sign, @user_password, "hex", "hex")
 end
